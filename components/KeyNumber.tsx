...
const KeyNumber = ({ number, description, animate = true }: { number: string, description: string, animate?: boolean }) => {
    ...
    return (
        <div ref={numberRef} className="col-span-2 gap-[24px]">
            <h3 className="text-2xl mb-2">
                {hasAnimated ? `${prefix}${count}${suffix ? ' ' + suffix : ''}` : `${prefix}0${suffix ? ' ' + suffix : ''}`}
            </h3>
            <p className="text-base">{description}</p>
        </div>
    );
